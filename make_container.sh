...
sudo docker run -d --name $TAG -p $DOCKER_HOST_PORT:$APP_PORT --restart unless-stopped -e $TIMEZONE -e $MODULE_NAME --log-driver json-file --log-opt max-size=$LOG_SIZE $TAG:$VER
docker ps -l
